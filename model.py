...
class GAN(object):
    ...
    def make_discriminator(self):
        ...
        X = tf.keras.layers.Conv2D(128, 5, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.GaussianNoise(0.1)(X)
        ...
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.GaussianNoise(0.1)(X)
        X = tf.keras.layers.Conv2D(1024, 5, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
    ...
    def make_generator(self):
        ...
        X = tf.keras.layers.Conv2DTranspose(1024, 5, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
