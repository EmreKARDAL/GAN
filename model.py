...
class GAN(object):
    ...
    def make_discriminator(self):
        ...
        X = tf.keras.layers.Conv2D(64, 3, strides=2, padding='same', use_bias=False)(image)
        ...
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Conv2D(512, 3, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Conv2D(1024, 3, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
    ...
    def make_generator(self):
        ...
        X = tf.keras.layers.Dropout(0.35)(generator_input)
        X = tf.keras.layers.Dense(4 * 4 * 1024, use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Reshape((4, 4, 1024))(X)
        X = tf.keras.layers.Conv2DTranspose(512, 3, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Conv2DTranspose(256, 3, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        ...
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Conv2DTranspose(3, 3, strides=2, padding='same', use_bias=False, activation='tanh')(X)
