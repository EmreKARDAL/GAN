...
class GAN(object):
    def __init__(self, image_wsize, image_hsize, caption_size, noise_size):
        ...
        self.g_optim = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.5, beta_2=0.9)
        self.d_optim = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.5, beta_2=0.9)
    ...
    def make_generator(self):
        ...
        label = tf.keras.layers.Input(shape=(self.caption_size,))
        noise2 = tf.keras.layers.Dense(8192)(noise)
        noise2 = tf.keras.layers.LeakyReLU()(noise2)
        generator_input = tf.concat([noise2, label], axis=1)
