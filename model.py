...
class GAN(object):
    ...
    def make_discriminator(self):
        ...
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.GaussianNoise(0.1)(X)
        X = tf.keras.layers.Conv2D(256, 5, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.GaussianNoise(0.1)(X)
        X = tf.keras.layers.Conv2D(256, 5, strides=1, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.GaussianNoise(0.1)(X)
    ...
    def make_generator(self):
        ...
        X = tf.keras.layers.Conv2DTranspose(512, 5, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Conv2DTranspose(256, 5, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Conv2DTranspose(256, 5, strides=1, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Conv2DTranspose(128, 3, strides=2, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
        X = tf.keras.layers.Conv2DTranspose(128, 3, strides=1, padding='same', use_bias=False)(X)
        X = tf.keras.layers.BatchNormalization()(X)
        X = tf.keras.layers.LeakyReLU()(X)
